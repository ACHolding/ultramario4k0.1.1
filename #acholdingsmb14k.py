...
Sprites base64 embedded; SFX procedural; OST = Koji Kondo SMB1 1-1 / 8-4 (procedural, files optional).
"""
...
# Koji Kondo SMB1 note frequencies (2A03 pulse)
_NOTE_FREQS = {
	'C3': 130.81, 'D3': 146.83, 'E3': 164.81, 'F3': 174.61, 'G3': 196.00, 'A3': 220.00,
	'Bb3': 233.08, 'B3': 246.94, 'C4': 261.63, 'D4': 293.66, 'E4': 329.63, 'F4': 349.23,
	'G4': 392.00, 'A4': 440.00, 'Bb4': 466.16, 'B4': 493.88, 'C5': 523.25, 'D5': 587.33,
	'E5': 659.25, 'F5': 698.46, 'G5': 783.99, 'A5': 880.00, 'Bb5': 932.33, 'B5': 987.77,
}
_DUTY_12 = 0.125
_MUSIC_SR = 44100

# World 1-1 overworld theme (100 BPM, sixteenth-note lengths)
_OVERWORLD_1_1 = [
...
_OVERWORLD_1_1 += _OVERWORLD_1_1[14:]

# World 8-4 castle / Bowser theme (faster pulse)
_CASTLE_8_4 = [
	('A4', 2), ('-', 2), ('A4', 2), ('-', 2), ('A4', 2), ('-', 2), ('A4', 2), ('-', 2),
	('F4', 2), ('-', 2), ('F4', 2), ('-', 2), ('F4', 2), ('-', 2), ('F4', 2), ('-', 2),
	('C5', 2), ('-', 2), ('C5', 2), ('-', 2), ('C5', 2), ('-', 2), ('C5', 2), ('-', 2),
	('E5', 2), ('-', 2), ('E5', 2), ('-', 2), ('E5', 2), ('-', 2), ('E5', 2), ('-', 2),
	('A4', 2), ('-', 2), ('A4', 2), ('-', 2), ('A4', 2), ('-', 2), ('A4', 2), ('-', 2),
	('F4', 2), ('-', 2), ('F4', 2), ('-', 2), ('F4', 2), ('-', 2), ('F4', 2), ('-', 2),
	('C5', 2), ('-', 2), ('C5', 2), ('-', 2), ('C5', 2), ('-', 2), ('C5', 2), ('-', 2),
	('E5', 2), ('-', 2), ('E5', 2), ('-', 2), ('E5', 2), ('-', 2), ('E5', 2), ('-', 2),
]
_CASTLE_8_4 += _CASTLE_8_4


def _apu_quantize(sample):
...
def _make_kondo_loop(notes, tempo=100, sr=_MUSIC_SR):
	"""Build looping NES-style pulse melody (Koji Kondo SMB1)."""
	...
	for note, length in notes:
		...
		for _ in range(dur):
			...
			if phase >= 1.0:
				phase -= 1.0
			mono.append(_apu_quantize(_apu_pulse(phase, _DUTY_12) * 0.52))
...
class APU:
	def __init__(self):
		self.enabled = True
		self.music_on = True
		self.use_files = False  # files=off: procedural Kondo OST only
	...
	def _init_kondo_ost(self):
		self._music_loops['1-1'] = _make_kondo_loop(_OVERWORLD_1_1, tempo=100)
		self._music_loops['8-4'] = _make_kondo_loop(_CASTLE_8_4, tempo=112)

	def _music_file_path(self, track_id):
		if not self.use_files:
			return None
		for ext in ('.ogg', '.mp3', '.wav', '.flac'):
			path = os.path.join(_MUSIC_DIR, track_id + ext)
	...
	def toggle_music_files(self):
		"""Toggle external OST files (music/1-1.*, music/8-4.*). Off = procedural only."""
	...
	def update_music(self, level_type='overworld', game_state='menu'):
		...
		if game_state != 'playing':
			self.stop_music()
			return
		track = '8-4' if level_type == 'castle' else '1-1'
...
class Game:
    ...
    def draw_menu(self):
        ...
        info = [
            ...
            f'M = Music {mus}   F = OST files {files}',
            'Koji Kondo SMB1: 1-1 (levels) / 8-4 (castle)',
            '',
